...
namespace AssetRipper.Core.Utils
{
	public class PGR
	{
		public static PGR Instance;
		private const string Header = "#$unity3dchina!@";
		private readonly byte[] KeyBytes = Encoding.UTF8.GetBytes(Keys[0]);
		...
		public PGR(EndianReader reader)
		{
			...
			data1 = ToUInt4Array(data1,0, data1.Length);
		...
		public byte[] ToUInt4Array(byte[] source, int offset, int size)
		{
		...
		public void DecryptDataSequence(byte[] bytes, int size, int index)
		{
			var offset = 0;
			do
			{
		...
		private (byte[], byte[]) ReadVector(EndianReader reader)
		{
